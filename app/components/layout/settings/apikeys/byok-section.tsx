...
export function ByokSection() {
  ...
  const saveMutation = useMutation({
    mutationFn: async ({
      ...
      if (!res.ok) throw new Error("Anahtar kaydedilemedi")
      return res.json()
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: async (_, provider) => {
      ...
      toast({
        ...
        description: `Senin ${providerConfig?.name} API anahtarı silindi.`,
      })
      await refreshAll()
    ...
    onError: (_, provider) => {
      const providerConfig = PROVIDERS.find((p) => p.id === provider)
      toast({
        title: "API anahtarı silinemedi",
        description: `Silinemedi ${providerConfig?.name} API anahtarı. Lütfen tekrar deneyin.`,
      })
      setDeleteDialogOpen(false)
      setProviderToDelete("")
    },
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground text-sm">
        Modellere erişimin kilidini açmak için kendi API anahtarlarınızı ekleyin.
      </p>
      <p className="text-muted-foreground text-sm">
        Anahtarlarınız uçtan uca şifreleme ile güvenli bir şekilde saklanır.
      </p>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {PROVIDERS.find((p) => p.id === providerToDelete)?.name} API anahtarı?
              Bu işlem geri alınamaz ve erişiminizi kaybedersiniz.{" "}
              {PROVIDERS.find((p) => p.id === providerToDelete)?.name} modeller.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
